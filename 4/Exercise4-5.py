...
print("What you like c to be?")
c = int(input())   # Speed of wave in meters per second
...
#make a u_past and check to see if c is postive or negative and use FITBIS or FITfIS for pos and neg respctivly
n = 0
...
for n in range(0, n_total): # start of time loop
    ...
    for i in range(1,nx-1):
        u_next[i] = u_now[i] - (sigma/24) * (23*(u_now[i+1] - u_now[i-1]) - 16 * (u_past[i+1] - u_past[i-1]) + 5*(u_ancient[i+1] - u_ancient[i-1]))
